...
class RunTaskUtils:
    ...
    def __run_interface(self, interface):
        ...
        text = InterfaceUtils.send_request(
                    "http://"+ url, interface.method, interface.headers, interface.parameter, interface.parameter_type)
        return text
    ...
    def __save_interface_result(self, interface, success, response, version):
        ...
        history.task_result = version.id
        history.interface_id = interface.id

        history.name = interface.name
        history.desctiption = interface.description
        ...
        history.header = interface.headers
    ...
    def run(self):
        ...
        interface = self.__get_task_interfaces()
        for i in interface:
